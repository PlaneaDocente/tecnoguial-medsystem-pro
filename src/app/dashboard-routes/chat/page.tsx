...
export default function ChatPage() {
  ...
  const sendMessage = useCallback(async () => {
    ...
    // Optimistic update con objeto parcial (sin id, se asigna en DB)
    const optimisticMessage: Partial<Message> & { id: string; conversation_id: string; sender_id: string; content: string; is_read: boolean; sent_at: string } = {
      id: `temp-${Date.now()}`,
    ...
    try {
      ...
      // Refrescar mensajes para obtener el ID real
              ...
              {filteredConversations.length === 0 ? (
                <div className="p-8 text-center">
                  ...
                  <p className="text-sm text-slate-500">
                    {search.trim() ? 'No se encontraron conversaciones' : 'No hay conversaciones'}
                  </p>
                  {!search.trim() && (
                ...
                filteredConversations.map(conv => (
                  <button
                    ...
                    className={`w-full p-4 flex items-center gap-3 hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors border-b border-slate-100 dark:border-slate-800 text-left ${
