...
export default function DashboardPage() {
  ...
  >([]);
  const [patientsEvolution, setPatientsEvolution] = useState<
    { month: string; count: number }[]
  >([]);

  // 🔥 FIX CRÍTICO: si la auth terminó y no hay usuario, salimos del spinner y redirigimos
  useEffect(() => {
    if (!authLoading && !user) {
  ...
  // 🔥 FIX CRÍTICO: cuando ya tenemos usuario, cargamos datos. Si no hay usuario pero auth terminó, paramos loading.
  useEffect(() => {
    if (authLoading) return; // esperamos a que useAuth resuelva primero

    if (user) {
      fetchDashboardData();
    } else {
      // No hay usuario y auth ya cargó: no nos quedamos colgados
      setLoading(false);
    }
  }, [authLoading, user]);
  ...
  // Mientras auth o dashboard cargan, mostramos spinner único
  ...
  // Si no hay usuario después de cargar, no renderizamos nada (el useEffect ya redirige)
  if (!user) return null;
            ...
            <div>
              ...
              <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">
                {todayAppointments.length > 0
                  ? `Próxima en ${todayAppointments[0]?.start_time}`
                  : "Sin más citas"}
              </p>
            </div>
            ...
            <div className="space-y-3">
              {todayAppointments.slice(0, 5).map((appointment) => (
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="font-medium text-slate-900 dark:text-white">
                        {(appointment.patient as unknown as { first_name: string; last_name: string })?.first_name}{" "}
                        {(appointment.patient as unknown as { last_name: string })?.last_name}
                      </p>
                  ...
                  <span
                    className={`px-2 py-1 rounded-full text-xs font-medium ${getAppointmentStatusColor(
                      appointment.status
                    )}`}
                  >
                    {appointment.status}
