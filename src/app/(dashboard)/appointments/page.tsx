        ...
        <div className="grid grid-cols-7 gap-1">
          {calendarDays.map((day, idx) => {
            ...
            return (
              <button
                key={idx}
                onClick={() => { setSelectedDate(dateISO); setSelectedAppointment(null); }}
                className={`
                ...
                `}
              >
                <span className={`text-sm font-medium ${isToday(day) ? 'text-blue-600' : ''}`}>{day.getDate()}</span>
                {hasAppointments && (
                  <div className="mt-1 space-y-1">
                    {dayAppointments.slice(0, 2).map(apt => (
                      <div key={apt.id} className={`text-xs px-1 py-0.5 rounded truncate ${getStatusColor(apt.status)}`} title={`${apt.start_time?.substring(0, 5)} - ${apt.patient?.first_name}`}>
                        {apt.start_time?.substring(0, 5)}
                      </div>
                    ))}
                    {dayAppointments.length > 2 && <div className="text-xs text-slate-500">+{dayAppointments.length - 2}</div>}
                  </div>
                )}
              </button>
            );
      ...
      {selectedDate && (
        <Card className="p-6">
          <h3 className="text-lg font-semibold text-slate-900 dark:text-white mb-4 capitalize">
            Citas del {parseLocalDate(selectedDate).toLocaleDateString('es-ES', { weekday: 'long', day: 'numeric', month: 'long' })}
          </h3>
              ...
              {getAppointmentsForDate(parseLocalDate(selectedDate)).map(apt => (
                ...
                >
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <p className="font-semibold text-slate-900 dark:text-white">{apt.patient?.first_name} {apt.patient?.last_name}</p>
          ...
          {selectedAppointment && (
            <div className="space-y-4">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <p className="font-semibold text-lg">{selectedAppointment.patient?.first_name} {selectedAppointment.patient?.last_name}</p>
              ...
              {selectedAppointment.notes && (
                <div><p className="text-sm text-slate-500 mb-1">Notas</p><p className="text-sm bg-slate-50 dark:bg-slate-800 p-3 rounded-lg">{selectedAppointment.notes}</p></div>
              )}
            </div>
          )}
          <DialogFooter className="gap-2 flex-wrap">
            {selectedAppointment?.status === 'pending' && (
              <Button onClick={() => selectedAppointment && updateAppointmentStatus(selectedAppointment.id, 'confirmed')}>
                <Check className="w-4 h-4 mr-2" aria-hidden="true" /> Confirmar
              </Button>
            )}
            {(selectedAppointment?.status === 'pending' || selectedAppointment?.status === 'confirmed') && (
              <Button variant="destructive" onClick={() => selectedAppointment && updateAppointmentStatus(selectedAppointment.id, 'cancelled')}>
                <X className="w-4 h-4 mr-2" aria-hidden="true" /> Cancelar
              </Button>
            )}
            {selectedAppointment?.status === 'confirmed' && (
              <Button onClick={() => selectedAppointment && updateAppointmentStatus(selectedAppointment.id, 'completed')}>
                <Check className="w-4 h-4 mr-2" aria-hidden="true" /> Completar
              </Button>
            )}
          </DialogFooter>
        </DialogContent>
